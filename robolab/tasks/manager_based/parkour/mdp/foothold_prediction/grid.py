"""Distribution decoding and quadrature for Gaussian foothold prediction."""
...
class FootholdGaussianGeometry:
    """Decode per-foot Gaussian parameters and provide expectation samples.

    Means are unconstrained base-frame XY values. Isotropic standard deviations
    are positive and bounded. A normalized square quadrature grid is shared by
    all samples and scaled by each predicted standard deviation.
    """
    ...
    def __init__(self, cfg, device: torch.device | str) -> None:
        ...
        if not 0.0 < self.sigma_min < self.sigma_max:
            raise ValueError(
                f"Expected 0 < sigma_min < sigma_max, got "
    ...
    def decode_distribution(
        ...
        """Decode raw ``[..., 3]`` values into mean XY and isotropic sigma."""
        mean = raw_distribution[..., :2]
    ...
    def expectation_points(
        ...
        """Return Gaussian quadrature points and normalized weights.

        Args:
            mean_xy: ``[..., 2]`` base-frame Gaussian means.
            sigma: ``[...]`` isotropic standard deviations in meters.
        """
